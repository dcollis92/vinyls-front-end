...
const App = () => {
  ...
  useEffect(() => {
    if (user) {
      profileService.getProfile(user.profile).then((profileData) => {
  ...
  const handleAddRecord = (record) => {
    profileService.addRecord(record).then((updatedProfile) => {
  ...
  const handleRemoveRecord = (profileId, recordsId) => {
    profileService.removeRecord(profileId, recordsId).then((updatedProfile) => {
      setProfile(updatedProfile);
    });
    window.location.reload();
  };
  ...
  const handleAddComment = (e, recordId, comment) => {
    ...
    recordService.addComment(recordId, comment).then((updatedRecord) => {
      setDbRecords(
        dbRecords.map((r) => (r._id === updatedRecord._id ? updatedRecord : r))
      );
    });
  };
  ...
  return (
    <>
      ...
      <Routes>
        ...
        />
        <Route path="/artist" element={<ArtistDetails records={records} />} />
        <Route
          path="/profile"
          element={
            <UserCollection
